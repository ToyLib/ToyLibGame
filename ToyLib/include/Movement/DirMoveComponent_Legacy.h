...
//------------------------------------------------------------------------------
// DirMoveComponent_Legacy
//------------------------------------------------------------------------------
// ・カメラ方向（ビュー基準）に対して移動する MoveComponent の派生。
// ・WASD / 左スティック入力を "カメラの向き" に合わせて前後左右へ移動する。
// ・移動後は進行方向へ自然に回転する（AdjustDir）。
//
// ※旧 DirMoveComponent の “完全コピー” 版（削除予定の退避用）
//------------------------------------------------------------------------------
class DirMoveComponent_Legacy : public MoveComponent
{
public:
    DirMoveComponent_Legacy(class Actor* owner, int updateOrder = 10);
    virtual ~DirMoveComponent_Legacy();
