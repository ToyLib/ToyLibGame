...
bool VKRenderer::CreateShadowResources()
{
    ...
    // already ok?
    if (mShadowExtent.width == w && mShadowExtent.height == h && mShadowDepthFormat != VK_FORMAT_UNDEFINED &&
        mShadowRenderPass != VK_NULL_HANDLE && mShadowSampler != VK_NULL_HANDLE &&
        (int)mShadowCascades.size() == kShadowCascadeCount && ShadowSceneReady() && !mShadowMapSet.empty())
    {
        return true;
    }
    ...
    for (int i = 0; i < kShadowCascadeCount; ++i)
    {
        ...
        if (!toy::vkutil::CreateImage2D(mPhysicalDevice, mDevice, w, h, mShadowDepthFormat, VK_IMAGE_TILING_OPTIMAL,
                                        VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                                        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, c.depthImg, c.depthMem,
        ...
        c.depthView =
            toy::vkutil::CreateImageView2D(mDevice, c.depthImg, mShadowDepthFormat, DepthAspect(mShadowDepthFormat));
        ...
        if (cmd)
        {
            toy::vkutil::CmdTransitionImageLayout(
                cmd, c.depthImg, DepthAspect(mShadowDepthFormat), VK_IMAGE_LAYOUT_UNDEFINED,
...
void VKRenderer::DestroyShadowResources()
{
    ...
    for (auto& c : mShadowCascades)
    {
        if (c.fb)
        {
            vkDestroyFramebuffer(mDevice, c.fb, nullptr);
        }
        c.fb = VK_NULL_HANDLE;

        if (c.depthView)
        {
            vkDestroyImageView(mDevice, c.depthView, nullptr);
        }
        c.depthView = VK_NULL_HANDLE;

        if (c.depthImg)
        {
            vkDestroyImage(mDevice, c.depthImg, nullptr);
        }
        c.depthImg = VK_NULL_HANDLE;

        if (c.depthMem)
        {
            vkFreeMemory(mDevice, c.depthMem, nullptr);
        }
...
void VKRenderer::UpdateShadowLightMatrices()
{
    ...
    const float orthoW[kShadowCascadeCount] = {mShadowOrthoWidth, mShadowOrthoWidth * 4.0f};
    const float orthoH[kShadowCascadeCount] = {mShadowOrthoHeight, mShadowOrthoHeight * 4.0f};

    for (int i = 0; i < kShadowCascadeCount; ++i)
    {
        Matrix4 lightProj = Matrix4::CreateOrtho(orthoW[i], orthoH[i], mShadowNear, mShadowFar);
...
void VKRenderer::DrawShadowPass()
{
    ...
    if (mShadowExtent.width == 0 || mShadowExtent.height == 0)
    {
        return;
    }
    ...
    for (int ci = 0; ci < kShadowCascadeCount; ++ci)
    {
        ...
        {
            toy::vkutil::CmdTransitionImageLayout(
                cmd, c.depthImg, DepthAspect(mShadowDepthFormat), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
        ...
        //------------------------------------------------------
        toy::vkutil::CmdTransitionImageLayout(
            cmd, c.depthImg, DepthAspect(mShadowDepthFormat), VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
...
void VKRenderer::TransitionShadowDepthToSampledIfNeeded(VkCommandBuffer cmd)
{
    ...
    for (int ci = 0; ci < kShadowCascadeCount; ++ci)
    {
        ...
        toy::vkutil::CmdTransitionImageLayout(
            cmd, mShadowCascades[ci].depthImg, DepthAspect(mShadowDepthFormat),
