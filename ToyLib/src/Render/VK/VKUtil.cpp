...
VkSampler CreateSampler_LinearClamp(VkDevice device)
{
    ...
    if (vkCreateSampler(device, &sci, nullptr, &s) != VK_SUCCESS)
    {
        return VK_NULL_HANDLE;
    }
    return s;
...
//--------------------------------------------------------------
// CreateImage1x1White_LinearHostVisible
//--------------------------------------------------------------
                                          ...
                                          VkImageView& outView)
{
    ...
    // Linear tiling は rowPitch などがあるが、1x1 なので offset に 4byte 書けばOK
    uint8_t* ptr = static_cast<uint8_t*>(mapped);
    const size_t off = static_cast<size_t>(layout.offset);
    ...
    outView = CreateImageView2D(device, outImg, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT);
    if (!outView)
    {
        vkDestroyImage(device, outImg, nullptr);
        vkFreeMemory(device, outMem, nullptr);
