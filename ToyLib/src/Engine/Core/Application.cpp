...
void Application::RunLoop()
{
    while (mIsActive)
    {
        ...
        UpdateFrame();
        {
            ...
            // ★Draw()の中にはGPU/Vsync待ち(fence待ち/画像取得/Presentやスワップ)
            //   が含まれており、これをRenderTimeMsに混ぜると実際の描画コストが
            //   見えなくなるため、レンダラー側で計測した待ち時間を差し引く。
    ...
    if (enable)
    {
        ...
        const bool foundMode =
            ...
            SDL_GetClosestFullscreenDisplayMode(
                displayId, mFullscreenWidth, mFullscreenHeight,
                0.0f, false, &closest);

        if (!foundMode || !SDL_SetWindowFullscreenMode(mWindow, &closest))
        {
