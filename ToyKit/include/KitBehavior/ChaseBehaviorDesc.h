...
{
    // 検知（Idle→Chase）は body 側の視界センサー（HumanoidDesc/CreatureDesc の
    // enableVision/visionFovDeg/visionMaxDist 等）で行う——検知距離はそちら側の
    // 設定であり、ここには無い。loseDistance は見失い判定（Chase→Idle）専用の
    // 基準距離で、検知距離とは別物（FleeBehaviorDesc::loseDistance と同じ位置付け）。
