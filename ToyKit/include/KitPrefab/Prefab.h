...
public:
    ...
    // 攻撃等でダメージを受けた「事実」を伝える。実際に何が起きるか（HPを引く等）は
    // 派生Prefabが自由に実装する（設計方針: Prefab自体はゲーム上の意味を持たない）。
    // 既定は何もしない（ダメージの概念を持たない Prefab はそのままでよい）。
    virtual void TakeDamage(int /*amount*/) {}

    // 撃破済みかどうか。HP等の概念を持たない Prefab は既定で false のまま。
